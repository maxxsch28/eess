...
if(!isset($_POST['idAsiento'])){
  ...
  $tmp = explode('_', $_POST['idAsiento']);
  if(is_numeric($tmp[1])){
    // muestra solo un asiento
    $sqlAsientos = trim("SELECT DISTINCT dbo.AsientosDetalle.idAsiento, Detalle, Fecha, IdRegistracionContable FROM dbo.AsientosDetalle, dbo.Asientos WHERE dbo.AsientosDetalle.idAsiento=dbo.Asientos.idAsiento AND dbo.Asientos.idAsiento=$tmp[1] order by fecha asc, idasiento asc;");
  } else {
