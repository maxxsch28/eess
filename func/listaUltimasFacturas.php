...
while($fila = $result->fetch_assoc()){
	...
	if(isset($fila['cantidadFacturas'])&&$fila['cantidadFacturas']>1){
		...
		foreach($numeros as $key => $n){
			...
			$enlaceModificar .= "&nbsp;<a href='/cargaMovistar.php?id=$ids[$key]'>$n</a>";
		}
		//$enlaceModificar = substr($enlaceModificar,0,-5);
	} else {
        $cantidadFacturas++;
		$multiple=false;
		$enlaceModificar = "<a href='/cargaMovistar.php?id=$fila[idFacturaRecibida]'>".substr($fila['numeroFactura'],-8)."</a>";
	}
...
elseif(!isset($limit)) $tabla.="<tr><td colspan='2' class='label-info center'>$cantidadClientes Clientes, $cantidadFacturas Facturas</td><td colspan='12' class='label-info center'>Total Facturas A: $totalA</td></tr><tr><td colspan='14' class='label-info center'>Total: ".($totalA+$totalB)."</td></tr>";
$result->close();
echo $tabla;
?>