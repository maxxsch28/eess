...
class loggedInUser {
	...
	public function updatepassword($pass)
	{
        ...
        if($this->remember_me == 1)updateSessionObj();
	...
	public function groupID()
	{
		...
		$sql = "SELECT users_users.group_id, 
			   users_groups.* 
			   FROM users_users
			   INNER JOIN users_groups ON users_users.group_id = users_groups.group_id 
			   WHERE
			   user_id  = '".$db->sql_escape($this->user_id)."'";
	...
	public function isGroupMember($id)
	{
		...
		$sql = "SELECT users_users.group_id, 
				users_groups.* FROM users_users 
				INNER JOIN users_groups ON users_users.group_id = users_groups.group_id
				WHERE user_id  = '".$db->sql_escape($this->user_id)."'
				AND
				users_users.group_id = '".$db->sql_escape($db->sql_escape($id))."'
				LIMIT 1
				";
