...
$totalB = 0;
$totalA = 0;
$cantidadFacturas = $cantidadClientes = $totalACapitalizar = 0;
$comision=array();
...
elseif(!isset($limit)) {
    ...
    foreach($comision[$idSocio] as $alicuota => $monto){
        ...
        if($a==1){
            ...
            $tablaEncabezado .= "<tr class='info comisionEncabezado'><td></td><td colspan='4' style='text-align:right'><b> {$_SESSION['transporte_tipos_comisiones'][$alicuota]}: $".number_format($monto, 2, ',', '.')."</b></td></tr>";
        }
    }
    if($a>1){
        // totaliza comisiones por fletero
