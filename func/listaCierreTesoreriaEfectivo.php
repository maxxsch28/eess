...
$sqlUltimoCierre = "select TOP 1 IdCierreCajaTesoreria, FechaCierre, Numero, ArqueoEfectivo, ArqueoChequesTerceros, UserName from dbo.CierresCajaTesoreria WHERE FechaCierre<='$aa-$mm-$dd 23:59:59' order by fechacierre desc;";
...
$sql['transferencias'] = "select Detalle, Fecha, Efectivo, IngresoEgreso from dbo.OtrosMovimientosCajaTesoreria WHERE (IdCierreCajaTesoreria>$ultimoCierre[IdCierreCajaTesoreria] OR IdCierreCajaTesoreria IS NULL) AND Efectivo>0 AND Fecha>='".$ultimoCierre['FechaCierre']->format('Y-m-d')."' AND Fecha<'$aa-$mm-$dd 23:59:59';";
...
$sql['ordenesPago'] = "select b.RazonSocial, PagoEfectivo, Numero, Fecha from dbo.OrdenesPago a, dbo.Proveedores b WHERE a.IdProveedor=b.IdProveedor AND Fecha>'".$ultimoCierre['FechaCierre']->format('Y-m-d H:i:s')."' AND Fecha<'$aa-$mm-$dd 23:59:59' AND (IdCierreCajaTesoreria>$ultimoCierre[IdCierreCajaTesoreria] OR IdCierreCajaTesoreria IS NULL) AND PagoEfectivo>0 AND IdFondoFijoCierre IS NULL;";
...
$jsonCierre[] = array('t' => 'Efectivo', 'clase' => 'bold', 'txt' => "Total", 'importe' => peso($efectivo));
$jsonCierre[] = array('t' => 'Cheques', 'clase' => 'bold', 'txt' => "Total", 'importe' => peso($cheques));
