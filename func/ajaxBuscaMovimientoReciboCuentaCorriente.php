...
echo $sqlPrincipal;
$stmt0 = odbc_exec( $mssql, $sqlPrincipal);
if( $stmt0 === false ){
...
while($rowPrincipal = odbc_fetch_array($stmt0)){
    ...
    while($rowRecibo = odbc_fetch_array($stmt)){
        ...
        $stmt2 = odbc_exec( $mssql, $sqlMovimientoImputado);
        if( $stmt2 === false ){
             echo "1. Error in executing query.</br>$sqlMovimientoImputado<br/>";
             die( print_r( sqlsrv_errors(), true));
        }
        $rowMovimientoImputado = odbc_fetch_array($stmt2);
