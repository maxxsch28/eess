...
$sqlUltimaMedicion = "SELECT * FROM cierres_cio_aforadores ORDER BY fechaCierre DESC LIMIT 1";
if($result = $mysqli->query($sqlUltimaMedicion)){
  $ultimaMedicion = $result->fetch_assoc();
  $result->close();
}
ChromePhp::log($ultimaMedicion);
...
$sqlYER = "select IdArticulo, SUM(Cantidad) as q from dbo.MovimientosFac, dbo.MovimientosDetalleFac where dbo.MovimientosFac.IdMovimientoFac=dbo.MovimientosDetalleFac.IdMovimientoFac and IdCliente=$clienteYER and Fecha>='".$ultimaFechaCargada->format('Y/m/d')." 22:00:00' AND Fecha<'".$ultimaFechaCargada->modify('+1 day')->format('Y/m/d')." 22:00:00' group by IdArticulo";
    ...
    <div class="container">
		...
		<form name='lecturaAforadores' id='lecturaAforadores' class='well'>
                ...
                <table class='table table-condensed' id='aforadores'>
                    ...
                    <thead>
                        <tr><th width='15%'>Pico</th><th width='22%'><?php echo "$fechaCierreAnterior[2]/$fechaCierreAnterior[1]/$fechaCierreAnterior[0]"?></th><th width='22%'><input type='text' name='fechaCierre' id='fechaCierre' class="input-sm form-control"  value="<?php echo $ultimaFechaCargada->format('d/m/Y')?>" data-date-format="dd/mm/yyyy" data-plus-as-tab='true'/></th><th width='20%'>Litros</th><th width='20%'><input type='checkbox' value='1' name='scontrol' id='scontrol' />S/Control</th></tr>
                    </thead>
                    <tbody>
                        <?php foreach($ultimaMedicion as $idPico => $aforador){
                          if(strpos($idPico, '_')){
                            $numeroPico = substr($idPico, strpos($idPico, '_')+1);
                            echo "<tr class='alert alert-".$classArticulo[$pico[$numeroPico]['idArticulo']]." art_{$pico[$numeroPico]['idArticulo']}'><td>$idPico</td>
                            <td><input type='text' id='ant_$idPico' name='ant_$idPico' class='input-sm form-control anterior' value='$aforador' disabled='disabled'/></td>
                            <td><input type='hidden' name='calc_$idPico' id='hcalc_$idPico'/><input type='number' min='0'  id='calc_$idPico' class='input-sm form-control' disabled='disabled'/></td>
                            <td><input type='number' min='0'  max='15000' step='any' id='$idPico' name='$idPico' class='input-sm form-control actual p{$pico[$numeroPico]['idArticulo']}' required='required' data-plus-as-tab='true'/></td>
                            <td><input type='number' min='0'  max='15000' step='any' id='sc_$idPico' name='sc_$idPico' class='input-sm form-control actual scontrol p{$pico[$numeroPico]['idArticulo']}' required='required' data-plus-as-tab='true' disabled='disabled'/></td></tr>";
                          }
                        }?>
                    </tbody>
                </table>
            ...
            <div class='col-md-2'>
              <?php 
                foreach($articulo as $IdArticulo => $datosArticulo){
                  echo "<h4 class='alert-".$classArticulo[$IdArticulo]."'>".ucwords(strtolower($articulo[$IdArticulo]['descripcion']))." <span id='tot$IdArticulo' class='pull-right'>&nbsp;0.00</span></h4>";
                }
              ?>
              <h4>&nbsp;</h4>
              <?php 
                foreach($articulo as $IdArticulo => $datosArticulo){
                  if($datosArticulo['premium']){
                    echo "<h4 class='alert-".$classArticulo[$IdArticulo]."'>".ucwords(strtolower($datosArticulo['familia']))." <span id='mix$datosArticulo[familia]' class='pull-right'>&nbsp;0.00</span>%</h4>";
                  }
                }
              ?>
            ...
            <div class='col-md-5 well'>
                <fieldset>
                <legend>Medición de tanques desde el CIO</legend>  
                <table class='table' id='tanques'>
                    ...
                    <tbody>
                        ...
                        <?php 
                        foreach($tanque as $IdTanque => $detalleTanque){
                          echo "<tr class='alert alert-{$classArticulo[$detalleTanque['idArticulo']]}'>
                            <td>$IdTanque - {$articulo[$detalleTanque['idArticulo']]['descripcion']}</td>
                            <td><input type='number' min='0'  max='2000000' step='any' id='ant_tq$IdTanque' name='ant_tq$IdTanque' class='input-sm form-control tanqueAnterior' disabled='disabled' value='".$penultimaMedicionTanques["tq".$IdTanque]."'/></td>
                            <td><input name='tq$IdTanque' id='tq$IdTanque' type='number' min='0' max='40000' required='required' class='input-sm form-control tanques' data-plus-as-tab='true' value='".(($ultimaMedicionTanques["tq$IdTanque"]>0)?$ultimaMedicionTanques["tq$IdTanque"]:$sqlLitrosCalculados[$IdTanque]['LitrosTotales'])."'/></td>
                            <td id='calc_$IdTanque'></td>
                            <td id='desvio_$IdTanque'></td></tr>";
                        }
                        ?>
                        </tr>
                    </tbody>
    ...
    $(document).ready(function() {
      $('#scontrol').click(function() {
        if($(this).attr('checked')){
          $('.scontrol').attr('disabled',false);
          $('.scontrol').val('0');
        } else {
          $('.scontrol').attr('disabled',true);
          $('.scontrol').val('');
        }
      });
      <?php 
      $a = "var articulo = [";
      $b = "var totales = {";
      foreach($articulo as $IdArticulo => $art){
          $a .= "'$IdArticulo', ";
          $b .= "'$IdArticulo':0, ";
      }
      $a = substr($a, 0, -2) . "];";
      ...
      function totalizaProductos(item, index, arr){
        var aa = 0;
        $('.p'+item).each(function(){
          if(!isNaN(this.value) &&  this.value.length!=0){
            //var aa = parseFloat($('#tot'+item).text().match(/\d+/)[0], 10);
            //var aa = parseFloat($('#tot'+item).html());
            aa += parseFloat(this.value);
          }
          //alert(aa);
        });
        $('#tot'+item).html(aa.toFixed(2));
        //alert(totales.item);
      }
      ...
      $('.actual').focusout(function() {
          ...
          var aforadorActual = actual+anterior+sc;
          $("#calc_" + pico).val(aforadorActual);
          $("#hcalc_" + pico).val(aforadorActual);
          articulo.forEach(totalizaProductos);
          
          $("#mixNAFTAS").html(($('#tot2076').val()*100/($('#tot2076').val()+$('#tot2078').val())).toFixed(1));
          $("#mixDIESEL").html(($('#tot2068').val()*100/($('#tot2068').val()+$('#tot2069').val())).toFixed(1));
      ...
      $('#cargaLectura').click(function() {
            var opciones= {
              ...
              url:       'func/cargaLecturaAforadoresCIO.php',         // override for form's 'action' attribute 
...
;
          if(today != '<?php echo $ultimaFechaCargada->format('Y-m-d'); ?>'){
