...
WHERE dbo.Clientes.IdCliente=dbo.movimientosCta.idCliente AND idmovimientoimputado IN (select IdMovimientoCta from dbo.movimientosCta where idmovimientofac IN (select IdMovimientofac from dbo.MovimientosFac where puntoVenta=$_POST[prefijo] and numero=$_POST[numero] and idtipomovimiento IN ('FAA', 'FAB')))";
