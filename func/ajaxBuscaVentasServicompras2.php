...
foreach($cajero as $id => $quien){
  if(isset($promedioEmpleadoUltimosMeses[$id])){
    $_SESSION['recibo'][$id][0]=$quien;
    echo "<tr><td align='left'>$quien[nombre]</td>";
    echo "<td class='ampliar2' id='{$id}_".$_POST['mes'].'01'."'>$ ".sprintf("%.2f",$cajero[$id][$_POST['mes']])."</td>";
    $totalMes = $totalMes + $cajero[$id][$_POST['mes']];
    $linea = "";
    $q=0;
    // Calculo promedio
    ...
    $promedio = ($promedioEmpleadoUltimosMeses[$id]>0)?sprintf("%.2f",$promedioEmpleadoUltimosMeses[$id]):'S/D';
    ...
    $variacion = ($promedio>0)?sprintf("%.1f",$ponderaVendedor*(($cajero[$id][$_POST['mes']]/$promedio)-1)*100):'';
