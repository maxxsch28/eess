...
class Estado{
  ...
  public function Tanques(){
    ...
    while($fila = sqlsrv_fetch_array($stmt)){
      if($fila['IdArticulo']>0){
        $this->tanque[$fila['IdTanque']]['idArticulo']=$fila['IdArticulo'];
        $this->tanque[$fila['IdTanque']]['capacidad']=$fila['Capacidad'];
        $this->tanque[$fila['IdTanque']]['nivelSuspender']=$fila['AdvertirSiNivelCombustibleMenorALitros'];
        $this->tanque[$fila['IdTanque']]['nivelPedir']=$fila['SolicitarCombustibleCuandoNivelMenorALitros'];
      } else {
        $this->tanque[$fila['IdTanque']]['litros']=$fila['Litros'];
        $this->tanque[$fila['IdTanque']]['agua']=$fila['Agua'];
        $this->tanque[$fila['IdTanque']]['ultimamedicion']=$fila['LastUpdated'];
      }
      if(isset($this->tanque[$fila['IdTanque']]['litros'])&&isset($this->tanque[$fila['IdTanque']]['capacidad'])){
        $this->tanque[$fila['IdTanque']]['disponible'] = $this->tanque[$fila['IdTanque']]['capacidad']-$this->tanque[$fila['IdTanque']]['litros'];
        $this->tanque[$fila['IdTanque']]['ocupacion'] = $this->tanque[$fila['IdTanque']]['litros']/$this->tanque[$fila['IdTanque']]['capacidad'];
      }
    }
    return $this->tanque;
  }
  ...
  public function Articulos(){
    global $mssql;
    $this->articulo = array();
    $sql = "select IdArticulo, a.Descripcion, ColorARGB, b.IdFamiliaArticulo, c.Descripcion as familia from dbo.articulos a, dbo.gruposarticulos b, dbo.FamiliasArticulos c where a.IdGrupoArticulo=b.IdGrupoArticulo and b.Combustible=1 and b.IdFamiliaArticulo=c.IdFamiliaArticulo;";
    $stmt = odbc_exec2($mssql, $sql);
    while($articulo = sqlsrv_fetch_array($stmt)){
      $this->articulo[$articulo['IdArticulo']]['idArticulo']=$articulo['IdArticulo'];
  ...
  public function Picos(){
    ...
    while($pico = sqlsrv_fetch_array($stmt)){
      if($pico['IdArticulo']>0){
        ...
        $this->pico[$pico['IdManguera']]['litrosDiario']=$pico['litros'];
        $this->pico[$pico['IdManguera']]['importeDiario']=$pico['importe'];
        $this->pico[$pico['IdManguera']]['qDespachos']=$pico['q'];
      }
    }
    return $this->pico;
...
print_r($comb->Picos());
echo "<br><br>";
print_r($comb->Tanques());

// magia Class 26/6/19
$tanques = $comb->Tanques();
$articulos = $comb->Articulos();
...
function muestraDetallesTanquesTelemedidos(){
    global $mssql, $articulo, $classArticulo, $mysqli, $CFG, $tanques, $articulos, $desde, $hasta;
    $tablaTanques="";
    
    foreach($tanques as $IdTanque => $datosTanque){
      ...
      $tablaTanques.="<tr$classNoVentas>"
        . "<td class='alert alert-{$classArticulo[$datosTanque['idArticulo']]}'>".ucwords(strtolower($articulos[$datosTanque['idArticulo']]['descripcion']))." <span class='badge '>$IdTanque</span></td>"
