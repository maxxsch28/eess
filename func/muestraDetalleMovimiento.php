...
$stmt = sqlsrv_query( $mssql, $sqlAsiento);
if( $stmt === false ){
     echo "1. Error in executing query.</br>$sqlAsiento<br/>";
     die( print_r( sqlsrv_errors(), true));
}
$rowAsiento = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC);
...
switch($tipo){
  case "VENTAS":
    ...
    if($rowAsiento['Descripcion']=='ND VARIAS EN CUENTA CORRIENTE'){
      ...
      $stmt2 = sqlsrv_query( $mssql, $sqlMovimiento);
...
//         $sqlAcreditacion = 
        ...
        if($rowMovimiento['idCliente']>0){
          ...
          $stmt3 = sqlsrv_query( $mssql, $sqlMovimiento);
  ...
  case "COBRANZAS":
    ...
    if(trim($rowAsiento['Nombre'])=='COBRANZAS01'){
      ...
      $stmt3 = sqlsrv_query( $mssql, $sqlCheque);
      ...
      $stmt2 = sqlsrv_query( $mssql, $sqlCheque);
...
//     //echo $sqlMovimiento;
    ...
    if(is_array($rowMovimiento)){
      ...
      //echo $sqlMovimiento2;
      $stmt3 = sqlsrv_query( $mssql, $sqlMovimiento2);
      ...
      $stmt3 = sqlsrv_query( $mssql, $sqlAnulado);
