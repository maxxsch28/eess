...
function destorySession($name)
{
	global $remember_me_length,$loggedInUser,$db,$db_table_prefix;
	   ...
	   if($loggedInUser->remember_me == 0) { 
		...
		{
			$db->sql_query("DELETE FROM users_sessions WHERE session_id = '".$loggedInUser->remember_me_sessid."'");
...
function updateSessionObj()
{
	...
	$db->sql_query("UPDATE users_sessions SET session_data = '".$newObj."' WHERE session_id = '".$loggedInUser->remember_me_sessid."'");
}
