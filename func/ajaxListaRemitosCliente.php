  ...
  if($stmt){
    ...
    while($rowRemito = odbc_fetch_array($stmt)){
      ...
      if($rowRemito['DocumentoCancelado']<>0){
        ...
        $rowRemitoCancelado = odbc_fetch_array(odbc_exec($mssql, "SELECT IdMovimientoFac, IdTipoMovimiento, PuntoVenta, Numero, Fecha, IdMovimientoCancelado, DocumentoCancelado, IdCliente FROM dbo.MovimientosFac WHERE IdMovimientoCancelado='$rowRemito[IdMovimientoFac]' AND IdTipoMovimiento IN ('RDV', 'NCA', 'NCB') ORDER BY IdTipoMovimiento, Fecha, PuntoVenta, Numero"));
