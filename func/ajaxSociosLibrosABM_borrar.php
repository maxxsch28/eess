  ...
  if(sqlsrv_num_rows($stmt)>0){
    ...
    if(intval($idObtenido[0])>0){
      ...
      foreach($_POST['acta'] as $key => $acta){
        $sql = "INSERT INTO dbo.[socios.actas] (idFoja, acta, fecha, altas, bajas, detalle, fojaFirmas) VALUES ('$idObtenido[0]', '$acta', '{$_POST['fecha'][$key]}', '{$_POST['altas'][$key]}', '{$_POST['bajas'][$key]}', '{$_POST['detalle'][$key]}', '{$_POST['fojaFirmas'][$key]}')";
        ChromePhp::log($sql);
        $stmt = odbc_exec2( $mssql4, $sql, __LINE__, __FILE__);
      }
    
      if (isset($_FILES['archivo'])) {
        $nombreArchivo = str_pad($idObtenido[0], 4, "0");
